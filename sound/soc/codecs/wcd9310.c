...
#if 0 //defined(CONFIG_US_EURO_SWITCH)
					 ...
					 bool checkpolling)
{
...
#endif
	struct tabla_priv *tabla = snd_soc_codec_get_drvdata(codec);
	pr_debug("%s: songyy enter US_EURO_switch_micbias\n",
				 __func__);
	...
	if (vddio_switch && !tabla->mbhc_micbias_switched &&
		...
		pr_debug("%s: songyy leave US_EURO_switch_micbias\n",
				 __func__);
}
#endif
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (strnstr(w->name, internal1_text, 30))
			snd_soc_update_bits(codec, micb_int_reg, 0xE0, 0x60); // ZTE 1st mic
		else if (strnstr(w->name, internal2_text, 30))
			snd_soc_update_bits(codec, micb_int_reg, 0x1C, 0x0C); // ZTE 2nd mic
...
/* ZTE_Audio_CJ_120530, chenjun, 2012-05-30, stop */
		}
	} else {
		pr_err("%s: Bad tabla private data\n", __func__);
	}
...
static irqreturn_t tabla_dce_handler(int irq, void *data)
{
	...
	if ((priv->in_gpio_handler) || tabla_hs_skip_btn_press(priv)) { // ZTE
...
tabla_codec_get_plug_type(struct snd_soc_codec *codec, bool highhph)
{
	...
	for (i = 0; i < num_det; i++) {
		...
		//zte_audio_gushenggao_20130122 for headset detect start
		#if 1
		vddioswitch =0;// (i == (num_det - 1)) || (i == (num_det - 2));
		#else
		vddioswitch = (i == (num_det - 1)) || (i == (num_det - 2));
		#endif
		//zte_audio_gushenggao_20130122 for headset detect end
		if (i == 0) {
...
static void tabla_hs_correct_gpio_plug(struct work_struct *work)
{
	...
	timeout = jiffies + msecs_to_jiffies(TABLA_HS_DETECT_PLUG_TIME_MS);
	...
	#if defined(CONFIG_US_EURO_SWITCH)
		pr_err("chenjun:%s: CONFIG_US_EURO_SWITCH songyy \n", __func__);
		...
		US_EURO_switch_micbias(tabla->codec, 1, false,
						     false);
		pr_err("chenjun:%s: enable micbias for US_EURO_SWITCH songyy \n", __func__);
		#endif
	#endif
				       ...
				       bool is_mb_trigger)
{
	...
	if (is_removal) {
		...
		tabla_cancel_hs_detect_plug(priv,&priv->hs_correct_plug_work_nogpio);
