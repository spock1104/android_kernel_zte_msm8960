...
static void bcm2079x_disable_irq(struct bcm2079x_dev *bcm2079x_dev)
{
	...
	dev_err(&bcm2079x_dev->client->dev,
			"bcm2079x_disable_irq bcm2079x_dev->irq_enabled = %d\n", bcm2079x_dev->irq_enabled);
...
static void bcm2079x_enable_irq(struct bcm2079x_dev *bcm2079x_dev)
{
	...
	spin_lock_irqsave(&bcm2079x_dev->irq_enabled_lock, flags);
	dev_err(&bcm2079x_dev->client->dev,
			"bcm2079x_enable_irq bcm2079x_dev->irq_enabled = %d\n", bcm2079x_dev->irq_enabled);
	if (!bcm2079x_dev->irq_enabled) {
...
static irqreturn_t bcm2079x_dev_irq_handler(int irq, void *dev_id)
{
	...
	dev_err(&bcm2079x_dev->client->dev,
			"bcm2079x_dev_irq_handler bcm2079x_dev->count_irq = %d\n", bcm2079x_dev->count_irq);
...
static unsigned int bcm2079x_dev_poll(struct file *filp, poll_table *wait)
{
	...
	dev_err(&bcm2079x_dev->client->dev,
			"bcm2079x_dev_poll bcm2079x_dev->count_irq = %d\n", bcm2079x_dev->count_irq);
				  ...
				  size_t count, loff_t *offset)
{
	...
	**/
	...
	if (total > count || copy_to_user(buf, tmp, total)) {
		dev_err(&bcm2079x_dev->client->dev,
			"bcm_nfc: failed to copy to user space, total = %d\n", total);
		total = -EFAULT;
				   ...
				   size_t count, loff_t *offset)
{
	...
	if (count > MAX_BUFFER_SIZE) {
		dev_err(&bcm2079x_dev->client->dev, "bcm_nfc out of memory\n");
		return -ENOMEM;
	}

	if (copy_from_user(tmp, buf, count)) {
		dev_err(&bcm2079x_dev->client->dev,
			"bcm_nfc failed to copy from user space\n");
		return -EFAULT;
	}
	...
	if(ret != count)
		dev_err(&bcm2079x_dev->client->dev,
			"bcm_nfc bcm2079x_dev_write ret = %d\n", ret);
	/*
	if (ret != count) {
...
static int bcm2079x_dev_open(struct inode *inode, struct file *filp)
{
	...
	dev_info(&bcm2079x_dev->client->dev,
		 "bcm_nfc device node major=%d, minor=%d\n", imajor(inode), iminor(inode));
					 ...
					 unsigned int cmd, unsigned long arg)
{
	...
	case BCMNFC_CHANGE_ADDR:
		dev_info(&bcm2079x_dev->client->dev,
			 "%s, bcm_nfc BCMNFC_CHANGE_ADDR (%x, %lx):\n", __func__, cmd,
			 arg);
		change_client_addr(bcm2079x_dev, arg);
		break;
	case BCMNFC_POWER_CTL:
		dev_info(&bcm2079x_dev->client->dev,
			 "%s, bcm_nfc BCMNFC_POWER_CTL (%x, %lx):\n", __func__, cmd,
			 arg);
	...
	case BCMNFC_WAKE_CTL:
		dev_info(&bcm2079x_dev->client->dev,
			 "%s, bcm_nfc BCMNFC_WAKE_CTL (%x, %lx):\n", __func__, cmd,
			 arg);
		gpio_set_value(bcm2079x_dev->wake_gpio, arg);
		break;
	default:
		dev_err(&bcm2079x_dev->client->dev,
			"%s, bcm_nfc unknown cmd (%x, %lx)\n", __func__, cmd, arg);
		return 0;
	}
			   ...
			   const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "%s, bcm_nfc probing bcm2079x driver flags = %x\n", __func__, client->flags);
	if (platform_data == NULL) {
		dev_err(&client->dev, "bcm_nfc probe fail\n");
		return -ENODEV;
	}

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dev_err(&client->dev, "bcm_nfc: need I2C_FUNC_I2C\n");
		return -ENODEV;
	}

	ret = gpio_request(platform_data->irq_gpio, "nfc_int");
	if (ret){
		printk("bcm_nfc: gpio_request(irq) fail !");		//zte-ccb-20121220
		return -ENODEV;
	}
	...
	if (ret){
		printk("bcm_nfc: gpio_request(ven) fail !");		//zte-ccb-20121220
		goto err_en;
	}
	...
	if (ret){
		printk("bcm_nfc: gpio_request(firm) fail !");	//zte-ccb-20121220
		goto err_firm;
	}
	...
	if (bcm2079x_dev == NULL) {
		dev_err(&client->dev,
			"bcm_nfc: failed to allocate memory for module data\n");
		ret = -ENOMEM;
		goto err_exit;
	}
	...
	if (ret) {
		dev_err(&client->dev, "bcm_nfc: misc_register failed\n");
		goto err_misc_register;
	}

	dev_info(&client->dev,
		 "%s, bcm_nfc: saving address %d\n",
	 ...
	 */
	dev_info(&client->dev, "bcm_nfc: requesting IRQ %d with IRQF_NO_SUSPEND\n", client->irq);
	bcm2079x_dev->irq_enabled = true;
	ret = request_irq(client->irq, bcm2079x_dev_irq_handler,
			  IRQF_TRIGGER_RISING|IRQF_NO_SUSPEND, client->name, bcm2079x_dev);
	if (ret) {
		dev_err(&client->dev, "bcm_nfc: request_irq failed\n");
		goto err_request_irq_failed;
	}
	bcm2079x_disable_irq(bcm2079x_dev);
	i2c_set_clientdata(client, bcm2079x_dev);
	dev_info(&client->dev,
		 "%s,bcm_nfc probing bcm2079x driver exited successfully\n",
		 __func__);
	return 0;
