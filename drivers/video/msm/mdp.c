					...
					struct mdp_histogram_data *hist)
{
	...
	ret = wait_for_completion_killable_timeout(&mgmt->mdp_hist_comp,
								timeout);
	if (ret <= 0) {
		printk("\n LCD TIME OUT %s,%d",__func__,__LINE__);
		if (!ret) {
			mgmt->hist = NULL;
			ret = -ETIMEDOUT;
...
int mdp_ppp_pipe_wait(void)
{
	...
	if (wait == TRUE) {
		ret = wait_for_completion_interruptible_timeout(&mdp_ppp_comp,
								5 * HZ);
		if (!ret)
			printk(KERN_ERR "LCD%s: Timed out waiting for the MDP.\n",
				__func__);
	}
...
static int mdp_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_MACH_GORDON  //video mode
		if(LcdPanleID ==LCD_PANEL_4P3_OTM1281_OT_TRULY) 
		{
	...
	if (!(mdp_pdata->cont_splash_enabled))//pan
		mdp_clk_ctrl(0);
	...
	if (mdp_pdata) {
		if (mdp_pdata->cont_splash_enabled &&
			...
			char *cp;
			uint32 bpp = 4; // 3;  //change to 4 for rgb8888
