 ...
 *
 * $Id: dhd_sdio.c 355555 2012-09-07 05:39:41Z $
 */
...
dhdsdio_htclk(dhd_bus_t *bus, bool on, bool pendok)
{
	...
	if (on) {
		...
		printk("shaohua test clkreq=%08x\n", clkreq);
		...
		printk("shaohua test clkctl = %08x \n", clkctl);


		/* Otherwise, wait here (polling) for HT Avail */
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only)) {

			printk("shaohua wait for clkd ready\n");
			SPINWAIT_SLEEP(sdioh_spinwait_sleep,
				((clkctl = bcmsdh_cfg_read(sdh, SDIO_FUNC_1,
			                                    SBSDIO_FUNC1_CHIPCLKCSR, &err)),
			          !SBSDIO_CLKAV(clkctl, bus->alp_only)), 100 * 10000); //PMU_MAX_TRANSITION_DLY);

			printk("shaohua wait finished\n");
		}
		if (err) {
			DHD_ERROR(("%s: HT Avail request error: %d\n", __FUNCTION__, err));
			return BCME_ERROR;
		...
		printk("shaohua test 22222 clkctl = %08x, alp=%08x \n", clkctl, bus->alp_only);
