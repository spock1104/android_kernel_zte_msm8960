...
//tcdtcd
struct mdp4_fps_statistic mdp4_fps_stat;
#endif
...
int64_t last_fps_update = 0;
int64_t this_fps_update = 0;
cputime64_t last_diff = 0;

extern bool vidc_playing;

#define GOOD    1 
#define BAD	-1
#define STILL_INTERVAL 300

int is_fps_ok(void)
{
#if defined (CONFIG_MACH_OKMOK)	
	cputime64_t diff;
	int64_t now = ktime_to_ms(ktime_get());

	diff = cputime64_sub((cputime64_t)now,(cputime64_t)this_fps_update);
	if (diff > STILL_INTERVAL)
		return GOOD;
	
	if ( (vidc_playing == true && diff > 54) || 
			(vidc_playing == false && (diff > 10 || last_diff > 18)))
		return BAD;
	else
		return GOOD;
#else
...
if (isr & INTR_PRIMARY_VSYNC) { 
	...
	this_fps_update = ktime_to_ms(ktime_get());
	last_diff = cputime64_sub((cputime64_t)this_fps_update,(cputime64_t)last_fps_update);
	last_fps_update = this_fps_update;

	if (last_diff <= 20){
		mdp4_fps_stat.fps_20ms++;
	} else if (last_diff <= 40) {
		mdp4_fps_stat.fps_40ms++;
	} else if (last_diff <= 60) {
		mdp4_fps_stat.fps_60ms++;
	} else {
		mdp4_fps_stat.fps_other++;
	}
	if ((last_diff > 60) && (vidc_playing == false) && (last_diff < 150)){
		__trace_printk(0, "fps delay %llu ms\n", last_diff);
	}
}//tcdtcd
#endif
