...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	switch(charger_type){
		case 	POWER_SUPPLY_TYPE_UNKNOWN:
			printk("charger_type = %d:POWER_SUPPLY_TYPE_UNKNOWN\n", charger_type);
		break;	

		case 	POWER_SUPPLY_TYPE_BATTERY:	/* Most time to print, so ignore it*/
			//printk("setting usb power supply type charger_type = %d:POWER_SUPPLY_TYPE_BATTERY\n",charger_type );
		break;

		case	POWER_SUPPLY_TYPE_UPS:
			printk("setting usb power supply type charger_type = %d:POWER_SUPPLY_TYPE_UPS\n", charger_type);			
		break;

		case	POWER_SUPPLY_TYPE_MAINS:
			printk("setting usb power supply type charger_type = %d:POWER_SUPPLY_TYPE_MAINS\n", charger_type);
		break;

		case	POWER_SUPPLY_TYPE_USB:		/* Standard Downstream Port */
			printk("setting usb power supply type charger_type = %d:POWER_SUPPLY_TYPE_USB\n", charger_type);

		break;

		case	POWER_SUPPLY_TYPE_USB_DCP:	/* Dedicated Charging Port */
			printk("setting usb power supply type charger_type = %d:POWER_SUPPLY_TYPE_USB_DCP\n", charger_type);
		break;

		case	POWER_SUPPLY_TYPE_USB_CDP:	/* Charging Downstream Port */
			printk("setting usb power supply type charger_type = %d:POWER_SUPPLY_TYPE_USB_CDP\n",charger_type);			
		break;

		case	POWER_SUPPLY_TYPE_USB_ACA:	/* Accessory Charger Adapters */
			printk("setting usb power supply type charger_type = %d:POWER_SUPPLY_TYPE_USB_ACA\n", charger_type);
		break;

		case	POWER_SUPPLY_TYPE_BMS:		/* Battery Monitor System */
			printk("setting usb power supply type charger_type = %d:POWER_SUPPLY_TYPE_BMS\n", charger_type);
		break;

        default:
			printk("unknown type charger_type = %d:POWER_SUPPLY_TYPE_UNKNOWN\n", charger_type);
		break;			
...
static int skeep_invalid_chg_work = 0;
...
static void msm_invalid_chg_work(struct work_struct *w)
{
	...
	if(skeep_invalid_chg_work){
		skeep_invalid_chg_work = 0;
		return;
		}
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	pr_debug("usb msm_otg %s work, \n", otg_state_string(otg->phy->state));
	pr_err("usb msm_otg phy->state:%d, input:%lx, type:%d, chg_state:%d\n",otg->phy->state,motg->inputs,motg->chg_type,motg->chg_state);
