...
#define WLAN_REG_ON	14         //ZTE_WIFI_ZHP_20110829
	...
	switch (onoff) {
		...
		case WLAN_RESET_ON:
			...
			wifi_set_power(1, 0);
			        ...
			        gpio_direction_output(WLAN_REG_ON,0);
			        mdelay(10);
			        gpio_direction_output(WLAN_REG_ON,1);
			        mdelay(200);
#endif
		...
		//gpio_free(WLAN_WAKE_UP);//ZTE_WIFI_ZHP_20110829
		bcm_detect_card(0);
		printk("shaohua power off------------------------ \n");
#endif
		break;
...
#if 0
			if((gpio_request(WLAN_REG_ON,"shaohua")) < 0) {
				printk("request fail \n"); 
			} else {
				printk("request success \n");
				gpio_direction_output(WLAN_REG_ON,0);	
				printk("WLAN_REG_ON--->0\r\n");
				mdelay(10);
				gpio_direction_output(WLAN_REG_ON,1);
				mdelay(200);
				bcm_detect_card(0);
