		...
		if ((nr_cpu_online < 2) && (rq_depth >= NwNs_Threshold[index])) {
			if (total_time >= TwTs_Threshold[index]) {
				new_state = MSM_MPDEC_UP;
				  ...
				  /*If the clock of cpu0 is below the set idle_freq don't enable cpu1 and
                                disregard the runqueue value.*/
                                if (acpuclk_krait_get_rate((CONFIG_NR_CPUS - 2)) <=
...
extern unsigned long acpu_check_khz_value(long);
