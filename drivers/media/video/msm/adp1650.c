...
static int adp1650_setup(struct i2c_client *client)
{
	...
	ret = adp1650_write(client, ADP1650_REG_TIMER_IOCFG,
			    pdata->timer_iocfg);
	pr_err("%s: pdata->timer_iocfg= 0x%X\n", __func__,pdata->timer_iocfg);
	if (ret < 0)
		return ret;

	ret = adp1650_write(client, ADP1650_REG_CURRENT_SET,
			    pdata->current_set);
	pr_err("%s: pdata->current_set= 0x%X \n", __func__,pdata->current_set);
	if (ret < 0)
		return ret;

	ret = adp1650_read(client, ADP1650_REG_CURRENT_SET, &fault);
	pr_err("%s: read_CURRENT_SET= 0x%X\n", __func__,fault);


	ret = adp1650_write(client, ADP1650_REG_OUTPUT_MODE,
			    pdata->output_mode);
	pr_err("%s: pdata->output_mode= 0x%X \n", __func__,pdata->output_mode);
	if (ret < 0)
		return ret;

	ret = adp1650_write(client, ADP1650_REG_CONTROL, pdata->control);
	pr_err("%s: pdata->control= 0x%X \n", __func__,pdata->control);
	if (ret < 0)
		return ret;

	ret = adp1650_write(client, ADP1650_REG_AD_MODE, pdata->ad_mode);
	pr_err("%s: pdata->ad_mode= 0x%X\n", __func__,pdata->ad_mode);
	if (ret < 0)
		return ret;

	ret = adp1650_write(client, ADP1650_REG_BATT_LOW, pdata->batt_low);
	pr_err("%s: pdata->batt_low= 0x%X \n", __func__,pdata->batt_low);
	if (ret < 0)
		return ret;
...
int ADP1650_assistmode_enable(void)
{
	...
	rc = adp1650_write(adp1650_client, 0x03,0x73);	
...
int ADP1650_flashmode_enable(void)
{
  ...
  #if defined (CONFIG_MACH_OKMOK) || defined (CONFIG_MACH_PRINDLE)
//jzq add for pmic control20121107
#define PM_FLASH_EN_GPIO PM8921_GPIO_PM_TO_SYS(6)
static int pmic_gpio_flash_en(void)
{
       int rc=0;

	struct pm_gpio pmic_gpio_en = {
	...
	rc = pm8xxx_gpio_config(PM_FLASH_EN_GPIO, &pmic_gpio_en);

	if (rc) {
			pr_err("%s: pm8921 gpio %d config failed(%d)\n",
					__func__, PM_FLASH_EN_GPIO, rc);
			return rc;
		  }
	
	rc = gpio_request(PM_FLASH_EN_GPIO, "pmic_gpio_en");
		if (rc) {
			pr_err("%s: unable to request gpio %d (%d)\n",
					__func__, PM_FLASH_EN_GPIO, rc);
			return rc;
		}
	gpio_set_value_cansleep(PM_FLASH_EN_GPIO, 0);
...
static int __devinit adp1650_probe(struct i2c_client *client,
	...
	if (chip->pdata->setup) {
		pr_err("chip->pdata->setup is ture\n");
  ...
  #if defined (CONFIG_MACH_OKMOK) || defined (CONFIG_MACH_PRINDLE)
              ret=pmic_gpio_flash_en();
	   
              if (!ret) {
			pr_err("%s: flash en OK\n", __func__);
			mdelay(1);
		}else{
			pr_err("%s: flash en failed\n", __func__);
			}
#else
		ret = gpio_request(2,"FLASH_NAME");
		if (!ret) {
			pr_err("%s: gpio_request1 2 OK\n", __func__);
			...
			ret = gpio_request(2,"FLASH_NAME");
			if (!ret){
			pr_err("%s: gpio_request 2 OK\n", __func__);
			gpio_direction_output(2,1);
			usleep_range(1000, 2000);
			mdelay(1);
			}else{
			pr_err("%s: gpio_request 2 failed\n", __func__);
			}
		}
...
static int adp1650_suspend(struct device *dev)
{
	...
	if (chip->pdata->setup) {
		...
		if (ret) {
			pr_err("%s setup failed",__func__);
			return ret;
		}
	}
