...
static int is_settingparam =0;
static int is_preview =0;
//static int is_snapshot =0;
static bool is_vreg_powerup=false;

static int isp_boot_flags=0;
//static struct regulator *ldo116;
static uint16_t iso_info_kerenl=0;
int iso_h=0;
int iso_l=0;
...
#define M7MO_CUSTOM_DEFAULT_POLL_COUNT		80		// 300 times
...
int g_switch_w=0;
unsigned long ispfw_size=0;
unsigned long ispfw_wsize=0;

 int read_iso_current_value(void);
...
int read_iso_current_value(void)
{
   ...
   int temp=0;
	...
	pr_err("%s ",__func__);
	iso_l=M7MO_ReadOneByteCRAM(0x07, 0x29);
	iso_h=M7MO_ReadOneByteCRAM(0x07, 0x28);
	
	pr_err("iso iso_h=0x%x ,iso_l=0x%x	",iso_h,iso_l);
	temp=(iso_h<<8|iso_l);

	pr_err("iso temp=%d ",temp);
...
/*
	...
	*/
	...
	},*/
	{
    	.x_output = 4320,
        .y_output =  3240,
        .line_length_pclk = 4320,
        .frame_length_lines =3240,
...
static enum msm_camera_vreg_name_t isp_veg_seq[] = {
	CAM_VIO,
	//CAM_VDIG,
	CAM_VAF,
};
...
int klib_fprintf(struct file *filp, const char *fmt, ...)
{
 ...
 int isp_cam_i2c_rxdata(unsigned short saddr,
                                       ...
                                       int length)
{
    ...
    if (i2c_transfer(isp_i2c_client->adapter, msg, 1) < 0)
    {
        pr_err("%s: failed!\n", __func__);
        return -EIO;
    }
...
unsigned char M7MO_ReadOneByteCRAM(unsigned char cat,unsigned char byte)
{
	...
	rc =isp_cam_i2c_rxdata(0x1f, cmd_senddata, cmd_recdata, 2);
    if (rc < 0)
    {
...
unsigned short M7MO_ReadOneHalfwordCRAM( unsigned char cat, unsigned char byte )
{
	...
	rc = isp_cam_i2c_rxdata( 0x1f,cmd_senddata, cmd_recdata, 3);
    if (rc < 0)
    {
        pr_err("%s: cat = 0x%x, byte = 0x%x, failed!\n", __func__, cat, byte);
    }	
...
unsigned long M7MO_ReadOneWordCRAM( unsigned char cat, unsigned char byte )
{
	...
	rc = isp_cam_i2c_rxdata( 0x1f,cmd_senddata, cmd_recdata, 5);
    if (rc < 0)
    {
        pr_err("%s: cat = 0x%x, byte = 0x%x, failed!\n", __func__, cat, byte);
    }	
...
void M7MO_ReadBytesCRAM( unsigned char cat, unsigned char byte , unsigned char byteCnt, unsigned char* cmd_recdata)
{
	...
	rc = isp_cam_i2c_rxdata(0x1f, cmd_senddata,  cmd_recdata, byteCnt+1);
    if (rc < 0)
    {
        pr_err("%s: cat = 0x%x, byte = 0x%x, failed!\n", __func__, cat, byte);
    }		
...
int M7MO_VER(void)
{
   int isp_ver_h=0;
   int isp_ver_l=0;
   int isp_ver_soft_h=0;
   int isp_ver_soft_l=0;

   char isp_ver[30]={0};

    isp_ver_h=M7MO_ReadOneByteCRAM(0x00, 0x01);
	pr_err("0x00, 0x01 = 0x%02x \n",isp_ver_h);
	isp_ver_soft_h=M7MO_ReadOneByteCRAM(0x00, 0x02);
	pr_err("0x00, 0x02 = 0x%02x \n",isp_ver_soft_h);
	isp_ver_soft_l=M7MO_ReadOneByteCRAM(0x00, 0x03);
	...
	snprintf(isp_ver,25, "isp ver: 0x%02x%02x\n", isp_ver_soft_h,isp_ver_soft_l);

	pr_err("%s len=%d",isp_ver,strlen(isp_ver));
...
static void M7MO_FwWriteMain( unsigned long send_buf, unsigned long send_buf_size)
{
  int	ret, loop;
	...
	while( flash_addr < flash_end ){
		...
		ispfw_wsize +=write_unit;
		pr_err("write_size =%ld ,total_size=%ld",ispfw_wsize,ispfw_size);
	}
	return ;
...
{	
	...
	sprintf(filepath, isp_cam_state?("/sdcard/%s"):("/system/etc/%s"), "RS_M7MO.bin");
 ...
 */
void M7MO_SingleCap_GetMain(int res)
{
    int stil_image_size=0x2F;
   ...
   if(res==2)
   	stil_image_size=0x27;
	M7MO_WriteOneByteCRAM(0x0B, 0x01, stil_image_size);		// 4000 x 3000(12M)
	

	pr_err("wt set image size 0x%x \n",stil_image_size);
#endif
...
void M7MO_CAF_Start(void )
{
	...
	//Start caf mode
    M7MO_WriteOneByteCRAM(0x0A, 0x4D, 0x00);

	M7MO_WriteOneByteCRAM(0x0A, 0x4A, 0x70); 
	//Start auto focus
	M7MO_WriteOneByteCRAM(0x0A, 0x02, 0x01);

}
void M7MO_VIDEO_CAF_Start(void )
{
	pr_err("%s	   ",__func__);
	...
	//Start caf mode
    M7MO_WriteOneByteCRAM(0x0A, 0x4D, 0x01);
	//Start auto focus
	M7MO_WriteOneByteCRAM(0x0A, 0x02, 0x01);

}

void M7MO_CAF_Trigger(void )
{
	pr_err("%s	   ",__func__);
...
{
    int width=(g_switch_w<2)?640:1280;
...
{
    int height=(g_switch_w<2)?480:720;
						...
						const char *buf, size_t count)
{
   ...
   {
    pr_err("echo buf=%s \n",buf);
	if(!strncmp(buf, "usr", 3))
	{
    isp_cam_state=1;
	pr_err("isp_cam_state =%d ",isp_cam_state);
	}

    pr_err("show_version =%d ",show_version);
	if(show_version==1)
	{
	isp_boot_flags=1;
	s_ctrl = (struct msm_sensor_ctrl_t *)(isp_id->driver_data);
	msm_isp_cam_sensor_power_down(s_ctrl);
...
static int isp_cam_powerdown(struct msm_sensor_ctrl_t *s_ctrl)
{
			...
			//is_vreg_powerup=false;
			rc=gpio_direction_output(46, 1);
		       rc=gpio_direction_output(97, 0);
...
static int isp_cam_powerup(struct msm_sensor_ctrl_t *s_ctrl)
{
		...
		rc=gpio_direction_output(97, 1);
		pr_err("%s  gpio 97\n",__func__);
	
		rc=gpio_direction_output(46, 0);
		pr_err("%s  gpio 46 \n",__func__);
			...
			int update_type, int res)
{
	...
	#endif
	   pr_err(" res=0x%x mode_num=0x%0x\n",res ,mode_num);
        //isp preview
		if(res==1 || res==3||((mode_num==11)&&(res==0)) ){

		touchafae_state=0;
		 
	
		    monito_size = (res==1)?(0x17):(0x25);
			if(((mode_num==11)&&(res==0)))
			{	
				monito_size=0x2A;
				burst_num=0;
			}else{
		    ...
		    pr_err("wt write monito_size=0x%x\n",monito_size);
			if(res != is_settingparam ||mode_num == 11)
			{
				M7MO_ChangeMode(E_M7MO_Mode_Parameter);
			...
			M7MO_WriteOneByteCRAM(0x01, 0x01,monito_size);
			M7MO_ChangeMode(E_M7MO_Mode_Monitor);
	 ...
	 case 4:
	  ...
	  case 5:
	   ...
	   case 6:
		 ...
		 M7MO_WriteOneByteCRAM(0x02, 0x10, 0x01);
		 M7MO_WriteOneByteCRAM(0x02, 0x0F, 0x06);
	   break;

	   case 7:
		 pr_err("%s saturation  7 \n",__func__);

		 M7MO_WriteOneByteCRAM(0x02, 0x10, 0x01);
		 M7MO_WriteOneByteCRAM(0x02, 0x0F, 0x07);
	   break;

	   case 8:
		 pr_err("%s saturation  8 \n",__func__);

		 M7MO_WriteOneByteCRAM(0x02, 0x10, 0x01);
		 M7MO_WriteOneByteCRAM(0x02, 0x0F, 0x08);
	   break;

	   case 9:
		 pr_err("%s saturation  9 \n",__func__);

		 M7MO_WriteOneByteCRAM(0x02, 0x10, 0x01);
		 M7MO_WriteOneByteCRAM(0x02, 0x0F, 0x09);
	   break;
	   
 	   case 10:
		 pr_err("%s saturation  10 \n",__func__);

		 M7MO_WriteOneByteCRAM(0x02, 0x10, 0x01);
		 M7MO_WriteOneByteCRAM(0x02, 0x0F, 0x0a);
	   break;   
		...
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int value)
{
	...
	case 5:
		//Set  video caf mode
		M7MO_VIDEO_CAF_Start();
		caf_state=1;
		pr_err("isp   %s  Set  video caf mode value:  %d\n", __func__,value);
	break;
		...
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int value)
{
	...
	case 2:
		...
		M7MO_WriteOneByteCRAM(0x0B, 0x39, 0x01); 
		break;
		...
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int value)
{
	...
	switch (value)
    {
		...
		case 19:
			pr_err("CAMERA_BESTSHOT_TEXT");
			M7MO_WriteOneByteCRAM(0x02, 0x59, 0x0D);
		break;
		...
		struct msm_sensor_v4l2_ctrl_info_t *ctrl_info, int value)
{
	...
	switch (value)
    {
	    case CAMERA_CONTRAST_LV1:
			...
			M7MO_WriteOneByteCRAM(0x02, 0x25, 0x0a);
		break;
        ...
        case CAMERA_CONTRAST_LV3:
			...
			M7MO_WriteOneByteCRAM(0x02, 0x25, 0x08);
		break;
		...
		case CAMERA_CONTRAST_LV5:
			...
			M7MO_WriteOneByteCRAM(0x02, 0x25, 0x06);
		break;
		...
		case CAMERA_CONTRAST_LV7:
			...
			M7MO_WriteOneByteCRAM(0x02, 0x25, 0x04);
		break;
		...
		case CAMERA_CONTRAST_LV9:
			M7MO_WriteOneByteCRAM(0x02, 0x25, 0x02);
			pr_err("CAMERA_CONTRAST_LV8");
		break;

		case 10:
			M7MO_WriteOneByteCRAM(0x02, 0x25, 0x01);
			pr_err("CAMERA_CONTRAST_LV9");
		break;
...
{
		pr_err("%s:   E    %d\n", __func__, __LINE__);
		pr_err(" is_preview=%d  is_settingparam=%d line=%d ",is_preview,is_settingparam,__LINE__);
		if(is_preview==0)
		{
...
//use
	...
	pr_err("%s:   mode=%d   res=%d  %d\n", __func__,mode,res, __LINE__);
...
int32_t ispcam_msm_sensor_get_iso_info(struct msm_sensor_ctrl_t *s_ctrl,
		uint16_t *iso_info)
{//use
	int rc = 0;
	 iso_info_kerenl=read_iso_current_value();	
	pr_err("%s:   e    %d iso_info_kerenl=%d\n", __func__, __LINE__,iso_info_kerenl);
	*iso_info=iso_info_kerenl;
...
static struct msm_sensor_fn_t isp_cam_func_tbl = {
	...
	.sensor_get_iso_info = ispcam_msm_sensor_get_iso_info,
	.sensor_config = msm_sensor_config,//ispcam_msm_sensor_config,
...
static struct msm_sensor_reg_t isp_cam_regs = {
	...
	.num_conf = ARRAY_SIZE(isp_cam_dimensions),
};
