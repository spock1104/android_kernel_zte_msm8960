	...
	loff_t *ppos)
{
	...
	memset((char *)&mdp4_fps_stat, 0 , sizeof(mdp4_fps_stat));	/* reset */
#endif
...
#if defined (CONFIG_MACH_OKMOK)		
	//tcdtcd, this calculate the performance
	if ( mdp4_fps_stat.fps_20ms + mdp4_fps_stat.fps_40ms){
		ulong i,j,k,l;
		bp += len;
		dlen -= len;
		i = mdp4_fps_stat.fps_20ms;
		j = mdp4_fps_stat.fps_40ms;
		k = mdp4_fps_stat.fps_60ms;
		l = mdp4_fps_stat.fps_other;
		len = snprintf(bp, dlen, "\nscore: %08lu  %08lu\n", 
				10000*i/(i+j),
				10000*(i+j+k)/(i+j+k+l));
	}
...
#if defined (CONFIG_MACH_OKMOK)
//tcdtcd
static int mdp_fps_open(struct inode *inode, struct file *file)
{
	/* non-seekable */
	file->f_mode &= ~(FMODE_LSEEK | FMODE_PREAD | FMODE_PWRITE);
	return 0;
...
static int mdp_fps_release(struct inode *inode, struct file *file)
{
	return 0;
}

static ssize_t mdp_fps_write(
	struct file *file,
	const char __user *buff,
	size_t count,
	loff_t *ppos)
	...
	loff_t *ppos)
{
	...
	len = snprintf(bp, dlen, "fps20   : [%08lu]\n", mdp4_fps_stat.fps_20ms);
	bp += len;
	dlen -= len;

	len = snprintf(bp, dlen, "fps20-40: [%08lu]\n", mdp4_fps_stat.fps_40ms);
	bp += len;
	dlen -= len;

	len = snprintf(bp, dlen, "fps40-60: [%08lu]\n", mdp4_fps_stat.fps_60ms);
	bp += len;
	dlen -= len;

	len = snprintf(bp, dlen, "fps60-  : [%08lu]\n", mdp4_fps_stat.fps_other);
	bp += len;
	dlen -= len;
...
static const struct file_operations mdp_fps_fops = {
	.open = mdp_fps_open,
	.release = mdp_fps_release,
	.read = mdp_fps_read,
	.write = mdp_fps_write,
};
#endif
...
//tcdtcd
#if defined (CONFIG_MACH_OKMOK)
	if (debugfs_create_file("fps", 0644, dent, 0, &mdp_fps_fops)
			== NULL) {
		printk(KERN_ERR "%s(%d): debugfs_create_file: debug fail\n",
			__FILE__, __LINE__);
		return -1;
