...
struct scatterlist *g_sg0, *g_sg1;
#endif
...
static void msmsdcc_reset_dpsm(struct msmsdcc_host *host)
{
	...
	if (is_wait_for_tx_rx_active(host)) {
		...
		while (readl_relaxed(host->base + MMCISTATUS) &
			 ...
			 * still active.
			 */
//yeganlin,20130130,change the poll time from 1ms to 5ms
			//if (ktime_to_us(ktime_sub(ktime_get(), start)) > 1000) {
			if (ktime_to_us(ktime_sub(ktime_get(), start)) > 5000) {
...
void msmsdcc_start_polling(struct mmc_host *mmc)
{
	...
	if(host->enable_polling_timer==1)
		mod_timer(&host->polling_timer, jiffies + 2 * HZ);
}
static
...
static void msmsdcc_dump_sdcc_state(struct msmsdcc_host *host)
{
//yeganlin,20130130,add some dump information
#if 1
        struct mmc_request *mrq = host->curr.mrq;

        if(mrq){
		pr_err("ygl %s: cmd->error = %d, mrq->sbc is %s\n", 
        		mmc_hostname(host->mmc),
        		mrq->cmd->error, (mrq->sbc == NULL ? "NULL" : " NOT NULL"));

            if(mrq->data)
                	pr_err("ygl %s: data->stop is %s, data->error = %d\n", 
                		mmc_hostname(host->mmc),
                		(mrq->data->stop == NULL ? "NULL" : " NOT NULL"),mrq->data->error);
        }
#endif
...
//yeganlin,20130131,added for alloc memory for struct of scatterlist failure 
#ifdef CONFIG_MMC_MSM_CARD_PRE_ALLOC_MEMORY_FOR_SG
      if( host->pdev_id == 3) {// for SD CARD
             pr_err("ygl msmsdcc_probe()::alloc for sg\n");
		g_sg0 = kmalloc(sizeof(struct scatterlist)*mmc->max_segs, GFP_KERNEL);;
		if (ret)
                pr_err("ygl msmsdcc_probe()::alloc for g_sg0 failed!go on and alloc at Queue.c again.\n");
		g_sg1 = kmalloc(sizeof(struct scatterlist)*mmc->max_segs, GFP_KERNEL);;
		if (ret)
                pr_err("ygl msmsdcc_probe()::alloc for g_sg1 failed!go on and alloc at Queue.c again.\n");
      }
#endif
