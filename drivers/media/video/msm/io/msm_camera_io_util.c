		...
		int num_vreg_seq, struct regulator **reg_ptr, int config)
{
	...
	if (config) {
		...
		}else if(curr_vreg->type == REG_VS){               //add by jzq for test
				reg_ptr[j] = regulator_get(dev,
				curr_vreg->reg_name);
			if (IS_ERR(reg_ptr[j])) {
				pr_err("%s: %s get failed\n",
...
int msm_camera_config_bug_vreg(struct device *dev, struct camera_vreg_t *cam_vreg,
...
int msm_camera_enable_bug_vreg(struct device *dev, struct camera_vreg_t *cam_vreg,
		...
		int num_vreg_seq, struct regulator **reg_ptr, int enable)
{
	...
	if (enable) {
		...
		if(cam_vreg[j].type== REG_LDO){  //add by wxl for p896a25
		  ...
		  else if(cam_vreg[j].type== REG_GPIO)   //add by wxl for p896a25 
			{
			      gpio_camera = cam_vreg[j].op_mode;
				pr_err("wxl %s: gpio_request gpio_camera %d OK\n", __func__,gpio_camera);
				rc = gpio_request(gpio_camera,"sensor");
				if (!rc) {
					pr_err("wxl %s: gpio_request gpio_camera %d OK\n", __func__,gpio_camera);
					gpio_direction_output(gpio_camera,1);
					usleep_range(1000, 2000);
					mdelay(1);
				} else {
					gpio_free(gpio_camera);
					mdelay(1);
					rc = gpio_request(gpio_camera,"sensor");
					if (!rc){
						pr_err("%s: gpio_request gpio_camera %d OK\n", __func__,gpio_camera);
						gpio_direction_output(gpio_camera,1);
						usleep_range(1000, 2000);
						mdelay(1);
					}else{
						pr_err("%s: gpio_request gpio_camera %dfailed\n", __func__,gpio_camera);
					}
				}
			}
		  else if(cam_vreg[j].type== REG_VS){
		  	rc = regulator_enable(reg_ptr[j]);
			if (rc < 0) {
				pr_err("%s: %s enable failed\n",
					__func__, cam_vreg[j].reg_name);
		...
		if(cam_vreg[j].type == REG_LDO){ //add by wxl for p896a25
			...
			pr_err("%s: %s disable success \n",__func__, cam_vreg[j].reg_name);
				...
				if (!rc) {
					pr_err("%s: gpio_request gpio_camera %d OK\n", __func__,gpio_camera);
					gpio_direction_output(gpio_camera,0);
					usleep_range(1000, 2000);
					mdelay(1);
				} else {
					gpio_free(gpio_camera);
					...
					rc = gpio_request(gpio_camera,"sensor");
					if (!rc){
						pr_err("%s: gpio_request gpio_camera %dOK\n", __func__,gpio_camera);
						gpio_direction_output(gpio_camera,0);
						usleep_range(1000, 2000);
						mdelay(1);
						}else{
							pr_err("%s: gpio_request gpio_camera %d failed\n", __func__,gpio_camera);
						}
				}
		  }
		else if(cam_vreg[j].type== REG_VS){
		  	rc = regulator_disable(reg_ptr[j]);
			if (rc < 0) {
				pr_err("%s: %s disable failed\n",
					__func__, cam_vreg[j].reg_name);
				goto disable_vreg;
