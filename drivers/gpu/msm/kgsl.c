			...
			unsigned int cmd, void *data)
{
	...
	entry = kgsl_mem_entry_create();
	//pr_err("[bob debug] %s: kgsl_mem_entry_create() return %p\n", __func__, entry ); 
	...
	//pr_err("[bob debug] %s: kgsl_allocate_user() return 0x%08x\n", __func__, result ); 
	...
	if (result == 0) {
		...
		trace_kgsl_mem_alloc(entry);
		//pr_err("[bob debug] %s: allocated successfully result is %d\n", __func__, result ); 

	} else
		{
		kfree(entry);
		pr_err("[bob debug] %s: allocated failed result is %d\n", __func__, result ); 
		}
