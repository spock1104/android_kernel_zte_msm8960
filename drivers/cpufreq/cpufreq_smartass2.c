...
static unsigned int ramp_down_step_saving = 108000;
...
static unsigned long min_cpu_load_hifreq = 60;

/*
 * The minimum amount of time to spend at a frequency before we can ramp up.
 * Notice we ignore this when we are below the ideal frequency.
 */
#define DEFAULT_UP_RATE_US 9000
 ...
 */
#define DEFAULT_DOWN_RATE_US 9000; //tcd change from 99->9
 ...
 */
#define DEFAULT_SAMPLE_RATE_JIFFIES 3
...
//tcd add 1106
#define DEF_CPU_UP_SAMPLE_TIME		(180000)
#define DEF_CPU_DOWN_SAMPLE_TIME	(220000)
static unsigned int cpu_up_sample = DEF_CPU_UP_SAMPLE_TIME;
static unsigned int cpu_down_sample = DEF_CPU_DOWN_SAMPLE_TIME;

static unsigned int cpu_up_rq = 250;
static unsigned int cpu_down_rq = 160;
...
static atomic_t atomic_tmp = ATOMIC_INIT(1);

static atomic_t hotplug_lock = ATOMIC_INIT(0);
static atomic_t fps_lock = ATOMIC_INIT(1);
//tcd add end
...
extern bool vidc_playing;
//extern int g_charger_type;

int current_state = NORMAL;


static unsigned long get_rx_tx_bytes(void)
{
	...
	for(i=0;i<8;i++){
		sprintf (buf, "rmnet%1d", i);
		ndev = dev_get_by_name(&init_net, buf);
		if(ndev->flags & IFF_UP){
			struct rtnl_link_stats64 temp;
			const struct rtnl_link_stats64 *stats;

		       	stats = dev_get_stats(ndev, &temp);
			t += (unsigned long)stats->rx_bytes;
			t += (unsigned long)stats->tx_bytes;
		}
	}
	return t;
}
...
static cputime64_t last_check, this_check;
static unsigned long last_byte, this_byte;
static unsigned int last_suspended;

void check_and_switch(void)
{
	int t = NORMAL;
	unsigned int cpu;
	...
	if ( is_benchmark == true)
		t = DO_BENCHMARK;
	else if (vidc_playing == true)
		t = VIDEO_PLAYING;
	else if (suspended == 1) {
		/* retrieve the last 1s network load, if exceeed 50KB, we know
		 * that phone is in silent busy, set hiideal freq, and we check it 
		 * one second
		 */
		t = SCREENOFF;
		if (last_suspended == 0){
			last_check = ktime_to_ms(ktime_get());
			last_byte  = get_rx_tx_bytes();
			last_suspended = 1;
		} else {
			this_check = ktime_to_ms(ktime_get());
			if (( ((cputime64_t)this_check) - ((cputime64_t)last_check) ) > 1000){
				this_byte  = get_rx_tx_bytes();
				last_check = this_check;
				dprintk(SMARTASS_DEBUG_MYPRINT, "enter screen off , %lu - %lu\n", last_byte, this_byte);
				if ( ((this_byte - last_byte) > 50*1024) && last_byte ){
					t = HOTSPOT_SCREENOFF;
					dprintk(SMARTASS_DEBUG_MYPRINT, "enter screen off , networking mode\n");
				}
				last_byte = this_byte;
			}
		}
	...
	if ( current_state == DO_BENCHMARK){
		min_cpu_lock = num_possible_cpus();
		for_each_possible_cpu(cpu) {
			this_smartass = &per_cpu(smartass_info, cpu);
			this_smartass->ideal_speed = 1512000;
		}
		min_cpu_lock = num_possible_cpus();
		dprintk(SMARTASS_DEBUG_MYPRINT, "enter benchmark\n");
	} else if ( current_state == VIDEO_PLAYING) {
		max_cpu_lock = 1;
		for_each_possible_cpu(cpu) {
			this_smartass = &per_cpu(smartass_info, cpu);
			this_smartass->ideal_speed = 702000;
		}
		dprintk(SMARTASS_DEBUG_MYPRINT, "enter videoplaying\n");
		...
		dprintk(SMARTASS_DEBUG_MYPRINT, "enter normal\n");
	} else if (current_state == HOTSPOT_SCREENOFF) {
		for_each_possible_cpu(cpu) {
			this_smartass = &per_cpu(smartass_info, cpu);
			this_smartass->ideal_speed = 1242000;
		}
		sleep_ideal_freq = 1242000;

		dprintk(SMARTASS_DEBUG_MYPRINT, "enter hotspot\n");
	} else if (current_state == SCREENOFF) {
		for_each_possible_cpu(cpu) {
			this_smartass = &per_cpu(smartass_info, cpu);
			this_smartass->ideal_speed = DEFAULT_SLEEP_IDEAL_FREQ;
		}
		sleep_ideal_freq = DEFAULT_SLEEP_IDEAL_FREQ;

		dprintk(SMARTASS_DEBUG_MYPRINT, "enter normal sleep\n");
	}
}
...
static int check_fps(void)
{
	if (atomic_read(&fps_lock) == 0)
		return 0;
	else 
		return is_fps_ok();
}
...
//tcd end	
	...
	for(up_rate = 1; num_hist - 1 - up_rate >= 0; up_rate++){
		...
		freq = hotplug_history->usage[prev].freq;
		min_freq = min(min_freq, freq);
		
		avg_load+= hotplug_history->usage[prev].load[0];
		rq+=hotplug_history->usage[prev].rq;

		if ( time_diff > cpu_up_sample )//200ms
			break;
	}
	if ((time_diff <= cpu_up_sample) || (up_rate == cpu_up_rate))
		return 0;
	...
	if (check_fps()<=0){
	    if ((avg_load > max_cpu_load_hifreq) && (rq > up_rq) && (min_freq >= up_freq)) {
		dprintk(SMARTASS_DEBUG_UPDOWN, "[HOTPLUG IN OK] %s %u>%d && load %u %d\n",
			__func__, rq, up_rq, avg_load, min_freq);
		hotplug_history->num_hist = 0;
		return 1;
	    } else
		dprintk(SMARTASS_DEBUG_UPDOWN, "[HOTPLUG IN FAIL %d] %s %u>%d && load %u, fps_deny_cpu_up %d times\n",
			min_freq, __func__, rq, up_rq, avg_load, fps_ok_cpu_up_fail);

	} else if ((avg_load > max_cpu_load_hifreq) && (rq > up_rq) && (min_freq >= up_freq)) {
			fps_ok_cpu_up_fail++;
	}
	hotplug_history->num_hist = 0;
...
//tcd end	
	...
	for(down_rate = 1; num_hist - 1 - down_rate >= 0; down_rate++){
		...
		if ( time_diff > cpu_down_sample )//200ms
			break;
	}
	if ((time_diff <= cpu_down_sample) || (down_rate == cpu_down_rate))
		return 0;
...
//tcd add start
	...
	if ((rq < down_rq) || ( max_freq <= down_freq) || (avg_load < min_cpu_load_hifreq)) {
//tcd end
	...
	if (check_fps() > 0){
		...
		dprintk(SMARTASS_DEBUG_UPDOWN, "[HOTPLUG OUT fps(%d)] %s %d<=%d && %u<%d fps_allow_cpu_down %d times, load %u, timediff=%llu\n",
			check_fps(), __func__, max_freq, down_freq, rq, down_rq, fps_ok_cpu_down_ok, avg_load, time_diff);
		hotplug_history->num_hist = 0;
		return 1;
...
static void apply_hotplug_lock(void)
{
	...
	if ( flag > 0)
		queue_work_on(0, updown_wq, work);
	else
		queue_work_on(0, updown_wq, work);
}
...
//tcd end		
	...
	delta = cputime64_sub(update_time, this_smartass->freq_change_time);

	__trace_printk(0, "tracer fps_ok %d, delta %llu, cpu_load %d, old_freq %d,\n", fps_ok, delta, cpu_load, old_freq);//tcd
	if (((cpu_load > max_cpu_load && old_freq < this_smartass->ideal_speed) || 
	   (cpu_load > max_cpu_load_hifreq && old_freq >= this_smartass->ideal_speed)) && old_freq < policy->max)
	{
		if ( (old_freq >= this_smartass->ideal_speed && delta >= up_rate_us && fps_ok <= 0)  || 
			(old_freq < this_smartass->ideal_speed) ) {
			dprintk(SMARTASS_DEBUG_ALG,"smartassT @ %d ramp up: load %d (delta_idle %llu %d)\n",
				old_freq,cpu_load,delta_idle, fps_ok);
			this_smartass->ramp_dir = 1;
			work_cpumask_set(cpu);
			queue_work(up_wq, &freq_scale_work);
			queued_work = 1;
		 	total_freq_up++;
		 } else if (fps_ok > 0 && old_freq >= this_smartass->ideal_speed && delta > up_rate_us){//tcd
			fps_fail_freq_up++;
		 }
	}
	else if (((cpu_load < min_cpu_load_hifreq && old_freq > this_smartass->ideal_speed) ||
		  (delta >= down_rate_us && (cpu_load < min_cpu_load) && old_freq <= this_smartass->ideal_speed))
		  && old_freq > policy->min)
	{
...
//tcd add start we open one cpu once
...
//this means when only one cpu , first choice to open  (4-3 2-1)
	if (online == 1) {
		dprintk(SMARTASS_DEBUG_UPDOWN, "CPU_UP 1\n");
		...
		for_each_cpu_not(cpu, cpu_online_mask) {
			...
			if (cpu == 0)
				continue;
			dprintk(SMARTASS_DEBUG_UPDOWN, "CPU_UPP %d\n", cpu);
	 		__trace_printk(0, "tracer cpu up\n");//tcd
			cpu_up(cpu);
			break;
		}
	}
...
static void cpu_down_work(struct work_struct *work)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (atomic_dec_and_test(&this_smartass->g_cpudown_lock)){
			dprintk(SMARTASS_DEBUG_UPDOWN, "CPU_DOWN %d\n", cpu);
	 		__trace_printk(0, "tracer cpu down\n");//tcd
			cpu_down(cpu);
		} else 
			dprintk(SMARTASS_DEBUG_UPDOWN, "CPU_DOWN failed of freqing\n");
			
		atomic_inc(&this_smartass->g_cpudown_lock);
...
//tcd add this
		...
		if (!atomic_dec_and_test(&this_smartass->g_cpudown_lock)){
			atomic_inc(&this_smartass->g_cpudown_lock);
			printk(KERN_ERR "tcd cpu freq failed due to cpu is downing\n");
			continue;
		}
///tcd add end	
		...
		if (old_freq != policy->cur) {
			...
			if (old_freq > this_smartass->ideal_speed) {
				...
				if (new_freq > old_freq){ // min_cpu_load > max_cpu_load ?!
					new_freq = old_freq -1;
					new_freq = old_freq - ramp_down_step_saving;//tcdtcd
				}
			}
		...
		// do act ual ramp up (returns 0, if frequency change failed):
		 __trace_printk(0, "freq %d => %d\n", old_freq , new_freq);//tcd
...
static ssize_t show_max_cpu_load_hifreq(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%lu\n", max_cpu_load_hifreq);
}

static ssize_t store_max_cpu_load_hifreq(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input > 0 && input <= 100)
		max_cpu_load_hifreq = input;
	return count;
}
...
static ssize_t show_min_cpu_load_hifreq(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%lu\n", min_cpu_load_hifreq);
}

static ssize_t store_min_cpu_load_hifreq(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input > 0 )
		min_cpu_load_hifreq = input;
	return count;
}

static ssize_t show_cpu_up_sample(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", cpu_up_sample);
}

static ssize_t store_cpu_up_sample(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
...
static ssize_t show_cpu_down_sample(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", cpu_down_sample);
}

static ssize_t store_cpu_down_sample(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input > 0 )
		cpu_down_sample = input;
	return count;
}

static ssize_t show_cpu_up_rq(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", cpu_up_rq);
}

static ssize_t store_cpu_up_rq(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input > 0 )
		cpu_up_rq = input;
	hotplug_rq[0][1] = cpu_up_rq;
	return count;
}

static ssize_t show_cpu_down_rq(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", cpu_down_rq);
}

static ssize_t store_cpu_down_rq(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
...
// fps_lock is 1 means  > 918 , fps  change freq up/cpu up
// 0 means , fps no work
// 3 means , fps change freq up/down, cpu up/down
// default is 1
...
{
	return sprintf(buf, "fps failed freq-up %d of %d\n fps fail cpu-up - %d of %d, fps make cpu down %d time\n", 
		fps_fail_freq_up,  total_freq_up + fps_fail_freq_up, fps_ok_cpu_up_fail, total_cpu_up + fps_ok_cpu_up_fail, fps_ok_cpu_down_ok);
...
define_global_rw_attr(cpu_down_sample);
define_global_rw_attr(cpu_up_sample);
define_global_rw_attr(cpu_up_rq);
define_global_rw_attr(cpu_down_rq);
define_global_rw_attr(max_cpu_load_hifreq);
define_global_rw_attr(min_cpu_load_hifreq);
...
static struct attribute * smartass_attributes[] = {
	...
	&cpu_down_sample_attr.attr,
	&cpu_up_sample_attr.attr,
	&cpu_down_rq_attr.attr,
	&cpu_up_rq_attr.attr,
	&max_cpu_load_hifreq_attr.attr,
	&min_cpu_load_hifreq_attr.attr,
	NULL,
};
...
static int __init cpufreq_smartass_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		this_smartass->g_cpudown_lock =	atomic_tmp;
