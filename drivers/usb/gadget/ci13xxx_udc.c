...
{
        struct ci13xxx *udc = container_of(w, struct ci13xxx , uevent_work);
        if (NULL == udc) {
                return;
        }
...
static int ci13xxx_vbus_session(struct usb_gadget *_gadget, int is_active)
{
	...
	pr_info("usb %s, activate:%d, gadget_ready:%d, softconnect:%d\n",__func__, is_active,gadget_ready,udc->softconnect);		
	if (gadget_ready) {
		if (is_active) {
		...
		void __iomem *regs)
{
	...
	INIT_WORK(&udc->uevent_work, scsicmd_usbstate_offline);
	/*end*/
