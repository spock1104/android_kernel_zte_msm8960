	...
	struct msm_eeprom_data_t *edata)
{
	...
	//modified by zhang.yu_1 for support eeprom to imx091 sensor	
	uint8_t i = 0 ;
	static bool lsc_read_flag = FALSE;
	...
	//modified by zhang.yu_1 for support eeprom to imx091 sensor	
	if(edata->index == 0x02 && !lsc_read_flag && (strcmp(ectrl->i2c_driver->driver.name,"imx091_eeprom") == 0)){
		lsc_read_flag = TRUE;
		if(ectrl->eeprom_blk_tbl_size){
			for(i=0; i<ectrl->eeprom_blk_tbl_size; i++){
				ectrl->i2c_client.client->addr = ectrl->eeprom_blk_i2c_addr_tbl[i];
				//compatible with sunny eeprom map,the map is in the im091_eeprom.c
				if(*((uint16_t *)ectrl->data_mod_info->data) == 0x0001)
					ectrl->i2c_client.client->addr  +=  0x02;
				printk("%s, addr  is : %d  reg is : %d \n",__func__,ectrl->i2c_client.client->addr,ectrl->eeprom_blk_tbl[i].reg_addr);
				rc = msm_camera_eeprom_read
				(ectrl, ectrl->eeprom_blk_tbl[i].reg_addr,
				ectrl->eeprom_blk_tbl[i].dest_ptr, ectrl->eeprom_blk_tbl[i].num_byte,
				ectrl->eeprom_blk_tbl[i].convert_endian);
		...
		if (ectrl->func_tbl.eeprom_format_lsc_data != NULL)
			ectrl->func_tbl.eeprom_format_lsc_data();		
	}
	...
	const struct i2c_device_id *id)
{
	...
	//modified by zhang.yu_1 for support eeprom to imx091 sensor	
	/*
	if(e_ctrl_t->eeprom_blk_tbl_size){
		for(i=0; i<e_ctrl_t->eeprom_blk_tbl_size; i++){
			e_ctrl_t->i2c_client.client->addr = e_ctrl_t->eeprom_blk_i2c_addr_tbl[i];
			printk("addr  is : %d  reg is : %d \n",e_ctrl_t->i2c_client.client->addr,e_ctrl_t->eeprom_blk_tbl[i].reg_addr);
			rc = msm_camera_eeprom_read
			(e_ctrl_t, e_ctrl_t->eeprom_blk_tbl[i].reg_addr,
			e_ctrl_t->eeprom_blk_tbl[i].dest_ptr, e_ctrl_t->eeprom_blk_tbl[i].num_byte,
			e_ctrl_t->eeprom_blk_tbl[i].convert_endian);
