...
static int direction[6] = {0,1,2,1,0,1};
#else
static int direction[6] = {0,1,2,0,0,0};
#endif
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d %d %d %d\n", val, cali_value[0], cali_value[1], cali_value[2]);
}
			       ...
			       const char *buf, size_t size)
{
	...
	if (val) {
		...
		for( i = 0; i < CALI_TIMES; i++){
			...
			temp_xyz[2]+= xyz[2];
#ifdef DEBUG
		printk(KERN_INFO "%s %s xyz=%d xyz=%d xyz=%d\n", 
						LIS3DH_ACC_DEV_NAME, __func__, xyz[0],
							xyz[1], xyz[2]);			
			
		printk(KERN_INFO "%s %s temp_xyz=%d temp_xyz=%d temp_xyz=%d\n", 
						LIS3DH_ACC_DEV_NAME, __func__, temp_xyz[0],
							temp_xyz[1], temp_xyz[2]);			
		}
#endif		
		cali_value[0] = temp_xyz[0]/CALI_TIMES;
		cali_value[1] = temp_xyz[1]/CALI_TIMES;
		cali_value[2] = ((temp_xyz[2]/CALI_TIMES) - 1000);

#ifdef DEBUG
		printk(KERN_INFO "%s %s %d %d %d\n", LIS3DH_ACC_DEV_NAME, __func__, cali_value[0],
							cali_value[1], cali_value[2]);
#endif
...
static struct device_attribute attributes[] = {
  	__ATTR(data, 0644, attr_get_sensor_data, NULL),
	__ATTR(delay, 0644, attr_get_polling_rate, attr_set_polling_rate),
	__ATTR(range, 0644, attr_get_range, attr_set_range),
	__ATTR(direct, 0644, attr_get_direct, attr_set_direct),
	__ATTR(cali, 0644, attr_get_cali, attr_set_cali),
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	acc->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 6;
