...
#define JACK_US_EURO_SEL_GPIO 44
#else
...
static bool msm8960_swap_gnd_mic(struct snd_soc_codec *codec)
{
#if 0 //songyy for P896F10 us and euro headset detection
...
static const struct snd_soc_dapm_route common_audio_map[] = {
	 ...
	 * on FLUID.
	 */
	 //zte_audio_gushenggao_20130311 for headset pop noise,start
	 #if 1
	...
	#endif
	 //zte_audio_gushenggao_20130311 for headset pop noise,end
...
#define S(X, Y) ((TABLA_MBHC_CAL_PLUG_TYPE_PTR(tabla_cal)->X) = (Y))
	...
	S(v_hs_max, 2850);// chenjun for fusheng headset:orig:2400:to 2850
#undef S
...
void US_EURO_Switch(int enable)//songyy 
{
...
pr_err("%s:enable(%d)\n", __func__, enable);
    ...
    {
#if 0
        gpio_direction_output(us_euro_sel_gpio, 1);
#else
        gpio_set_value_cansleep(us_euro_sel_gpio, 1);
#endif
        pr_err("%s:enable\n", __func__);
    }
    else
...
#else
        gpio_set_value_cansleep(us_euro_sel_gpio, 0);
#endif
        pr_err("%s:disable\n", __func__);
    }
}
