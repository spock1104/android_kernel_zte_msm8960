...
struct mpu_private_data {
	...
	struct notifier_block hibernate_nb;
};

struct mpu_private_data *mpu_private_data;

int mpu_config_int_gpio(int int_gpio)
{
    int rc=0;

    uint32_t  gpio_config_data = GPIO_CFG(int_gpio,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA);

    rc = gpio_request(int_gpio, "gpio_mpu3050");
    if (rc) {
        printk( "%s: gpio_request(%#x)=%d\n",
   ...
   // mdelay(1);
    ...
    if (rc) {
        printk( "%s: gpio_direction_input(%#x)=%d\n",
                __func__, int_gpio, rc);
        return rc;
    }
				    ...
				    unsigned long event, void *unused)
{
	...
	if (is_in_hibernate()) {
	    printk(KERN_ERR"(%s:%d) in hibernate\n", __FUNCTION__, __LINE__);
		return NOTIFY_OK;
	}
...
static int mpu_hibernate_notifier_callback(struct notifier_block *nb,
				    unsigned long event, void *unused)
{
	struct mpu_private_data *mpu =
	    container_of(nb, struct mpu_private_data, hibernate_nb);
	struct i2c_client *client = mpu->client;
	struct timeval event_time;
	dev_dbg(&client->adapter->dev, "%s: %ld\n", __func__, event);
	...
	mutex_lock(&mpu->mutex);
	if (!(mpu->pid) ||
	    (event != ZTE_HIBERNATE_ON && event != ZTE_HIBERNATE_OFF)) {
		mutex_unlock(&mpu->mutex);
		return NOTIFY_OK;
	}

	if (event == ZTE_HIBERNATE_ON)
		mpu->event = MPU_PM_EVENT_SUSPEND_PREPARE;
	if (event == ZTE_HIBERNATE_OFF)
		mpu->event = MPU_PM_EVENT_POST_SUSPEND;
...
static int inv_slave_read_tst(struct mldl_cfg *mldl_cfg,
			  void *gyro_adapter,
			  void *slave_adapter,
			  struct ext_slave_descr *slave,
			  struct ext_slave_platform_data *pdata,
...
//    unsigned char raw_data[6];	
	...
	case MPU_READ_ACCEL:
		...
		break;
	case MPU_READ_ACCEL_TST:
		dev_dbg(&client->adapter->dev, "%s: %d endia %d\n", __func__, cmd, mldl_cfg->slave[EXT_SLAVE_TYPE_ACCEL]->endian);
		retval = inv_slave_read_tst(
			mldl_cfg,
			slave_adapter[EXT_SLAVE_TYPE_GYROSCOPE],
			slave_adapter[EXT_SLAVE_TYPE_ACCEL],
			slave[EXT_SLAVE_TYPE_ACCEL],
...
int mpu_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	mpu->hibernate_nb.notifier_call = mpu_hibernate_notifier_callback;
	mpu->hibernate_nb.priority = 0;
	res = register_hibernate_notifier(&mpu->hibernate_nb);
	if (res) {
		dev_err(&client->adapter->dev,
			"Unable to register hibernate_notifier %d\n", res);
		goto out_register_hibernate_notifier_failed;
	}
	...
	if (client->irq) {
	  ...
	  mpu_config_int_gpio(client->irq);
...
out_whoami_failed:
	unregister_hibernate_notifier(&mpu->hibernate_nb);
out_register_hibernate_notifier_failed:
