...
static void mdp4_dsi_video_wait4dmap(int cndx)
{
	...
	if (atomic_read(&vctrl->suspend) > 0)
		{
		printk("LCD mdp4_dsi_video_wait4dmap");
		return;
		}
...
static void mdp4_dsi_video_wait4ov(int cndx)
{
	...
	if (atomic_read(&vctrl->suspend) > 0)
		{
		printk("LCD mdp4_dsi_video_wait4ov");
		return;
		}
...
int mdp4_dsi_video_off(struct platform_device *pdev)
{
	...
	msleep(20);	/* >= 17 ms */
	complete_all(&vctrl->vsync_comp);//move to 
...
//#ifdef CONFIG_MACH_ROUNDTOP
//	vctrl->vsync_irq_enabled  =0;//add for encrypt 
//#endif
	return ret;
}
