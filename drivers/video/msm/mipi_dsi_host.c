...
int mipi_dsi_cmd_dma_tx(struct dsi_buf *tp)
{
	...
	if(!wait_for_completion_timeout(&dsi_dma_comp,msecs_to_jiffies(40)))
...
void mipi_dsi_cmd_mdp_busy(void)
{
	...
	if (need_wait) {
		...
		if(!wait_for_completion_timeout(&dsi_mdp_comp,msecs_to_jiffies(40)))
		pr_err("LCD%s %d  TIMEOUT_\n", __func__, __LINE__);
	}
