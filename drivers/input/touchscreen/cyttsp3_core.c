...
static void cyttsp_timer_watchdog(struct work_struct *work)
{
	struct cyttsp *ts = container_of(work, struct cyttsp, work);
	int retval = 0;
       static u8 bHeartBeatCntPre = 0;
       u8 bHeartBeatCntCur = 0;
	struct cyttsp_xydata xy_data;
//	 printk("cyttsp_timer_watchdog\n");
//		_cyttsp_start_watchdog_timer(ts);	
//	return ;
	if (ts == NULL) {
		pr_err("%s: NULL context pointer\n", __func__);
		...
		if (ts->power_state == CY_ACTIVE_STATE) {
			...
			if (retval < 0) {
				...
				goto cyttsp_timer_watchdog_exit_error;
                     ...
                     //this is special treatment for N9500, heartbeat information locate in Z1's position.
                     bHeartBeatCntCur = xy_data.tch1.z;
                    if( bHeartBeatCntPre == bHeartBeatCntCur){
	                pr_err("bHeartBeatCntPre =%d,bHeartBeatCntCur=%d\n ",bHeartBeatCntPre, bHeartBeatCntCur);                        
                        bHeartBeatCntPre = 0;                        
			goto cyttsp_timer_watchdog_exit_error;
                    }
                    else{
           ...
           #if 0 
			if (GET_HSTMODE(xy_data.hst_mode) ==
				GET_HSTMODE(CY_SYSINFO_MODE)) {
				pr_err("%s: reset to op mode m=%02X\n",
...
if (retval < 0) {
	...
	ts->waiting_for_fw = true;
	retval = size;
	cyttsp_firmware_cont(cyttsp_fw,ts->dev);
		   ...
		   unsigned long count, void *data)
{
	...
	const char *fw_name ="ZTE_N9500_v05.bin";

	update_result_flag_cyttsp=0;

	if(ts)
		cyttsp_ic_reflash_store(ts->dev,NULL,fw_name,strlen(fw_name)+1);
