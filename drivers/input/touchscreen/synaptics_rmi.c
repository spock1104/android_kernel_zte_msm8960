		   ...
		   unsigned long count, void *data)
{
	...
	i2c_smbus_write_word_data(syn_ts->client, syn_ts->f11.ctrl_base+6,syn_ts->max[0]);
	i2c_smbus_write_word_data(syn_ts->client, syn_ts->f11.ctrl_base+8,syn_ts->max[1]);		
	enable_irq(syn_ts->client->irq);
	...
	)
{
	...
	case TS_POWER_ON:
		...
		*/
		ret = i2c_smbus_write_byte_data(ts->client, ts->f11.ctrl_base+2, 0x1);
		if (ret<0) pr_err("%s: i2c_smbus_write_byte_data failed\n",__func__);
		ret = i2c_smbus_write_byte_data(ts->client, ts->f11.ctrl_base+3, 0x1);
		if (ret<0) pr_err("%s: i2c_smbus_write_byte_data failed\n",__func__);
	...
	case TS_RESUME:	
		// set touchscreen size = lcd size
		i2c_smbus_write_word_data(ts->client, ts->f11.ctrl_base+6,ts->max[0]);
		i2c_smbus_write_word_data(ts->client, ts->f11.ctrl_base+8,ts->max[1]);
...
//	max_y = ts->max[1];
	...
	if ( ts->max_y_position != 0 )
		...
		ts->max[1] += ts->max_y_position;
	//printk("max_x = %d, max_y = %d \n", ts->max[0], ts->max[1] );

#ifdef TOUCHSCREEN_DUPLICATED_FILTER
	ts->dup_threshold=(ts->max[1]*10/yres+5)/10; 
