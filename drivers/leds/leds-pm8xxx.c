...
/* added by zhang.yu_1 for led blink  */
#ifdef CONFIG_LED_PWM_CTRL

#define LED_DELAY_ON_MIN		10
#define LED_DELAY_LEN_MIN	100

static u32 on_period;
		...
		struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "blink_delay_on = %ld ms \n", led_cdev->blink_delay_on);
	ret = strlen(buf) + 1;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (count == size) {
		ret = count;
		if(state < LED_DELAY_ON_MIN) state = LED_DELAY_ON_MIN;	  // min 10ms
		led_cdev->blink_delay_on = state;
		printk(LED_DRIVER_TAG "delay_on_store: %d\n",state);
	}else
		printk(LED_DRIVER_TAG "delay_on_store() write fail \n");
	
	return ret;
}

static ssize_t delay_len_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct led_classdev *led_cdev = dev_get_drvdata(dev);

	sprintf(buf, "blink_delay_off = %ld ms \n", led_cdev->blink_delay_off);	//blink_delay_off used for period
	ret = strlen(buf) + 1;

	return ret;
}

static ssize_t delay_len_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	char *after;
	unsigned int state;
	...
	if (count == size) {
		ret = count;
		if(state < LED_DELAY_LEN_MIN) state = LED_DELAY_LEN_MIN;	  // min 100ms
		led_cdev->blink_delay_off= state;
		printk(LED_DRIVER_TAG "delay_len_store: %d\n",state);
	}else
		printk(LED_DRIVER_TAG "delay_len_store() write fail \n");
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (count == size) {
		...
		if(led_cdev->blink_brightness != 0){	//led will blink

                        	pwm_disable(led->pwm_dev);	//ccb-debug
			if(led_cdev->blink_delay_on < LED_DELAY_ON_MIN|| led_cdev->blink_delay_off <LED_DELAY_LEN_MIN )
			{
				printk(LED_DRIVER_TAG "blink value error : blink_delay_on=%d; blink_delay_off=%d \n",
					(unsigned int )led_cdev->blink_delay_on,(unsigned int )led_cdev->blink_delay_off);
				
				led_cdev->blink_delay_on	= 502;	//special value to distinguish the init data !
				led_cdev->blink_delay_off 	= 1002;	//1000ms default !
			}
			if(led_cdev->brightness == 0)
				led_cdev->brightness = 3;	//make sure led blink

			on_period = (u32) (led_cdev->blink_delay_on * 1000);	//ms=>us
							
			led->pwm_period_us = (u32) (led_cdev->blink_delay_off * 1000) ;	//ms=>us	!!!!! blink_delay_off used for period !!!!
			
			printk(LED_DRIVER_TAG "blink_set: %s\t (on=%d,period=%d)\n",
				led->cdev.name,(unsigned int )led_cdev->blink_delay_on,(unsigned int )led_cdev->blink_delay_off);

		}else{	//led on off 
		
			on_period = led_cdev->brightness * 1000 / LED_FULL;	
				
			led->pwm_period_us = 1000;		//1000us=1ms
		
			printk(LED_DRIVER_TAG "light_set: %s\t(value=%d)\n",
				led->cdev.name,led_cdev->brightness);
		}
...
static DEVICE_ATTR(delay_on, 0644, delay_on_show, delay_on_store);
static DEVICE_ATTR(delay_len, 0644, delay_len_show, delay_len_store);
...
static int pm8xxx_led_pwm_work(struct pm8xxx_led_data *led)
{
	...
	if (led->pwm_duty_cycles == NULL) {
			...
			duty_us = on_period;	//us   zte-ccb-20120110
		
			//printk(LED_DRIVER_TAG "duty_us=%d; period=%d\n",on_period,led->pwm_period_us);
...
#ifdef CONFIG_LED_PWM_CTRL
	//zte-ccb-20120112 exclude red/green led ! do nothing for red/green
	if(!strcmp(led_cdev->name, "red") || !strcmp(led_cdev->name, "green"))	return;
#endif
...
static int __devinit pm8xxx_led_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pcore_data->num_leds; i++) {
		...
		#ifdef CONFIG_LED_PWM_CTRL//modified by zhang.yu_1 for led blink 
		//led_dat->cdev.blink_set              = pm8xxx_pwm_led_blink;   //zte-ccb-20120110
		#endif
...
#ifdef CONFIG_LED_PWM_CTRL		//zte-ccb-20120108
		
		// blink_delay_on & blink_delay_off for  red green led !
		if(led_dat->id == PM8XXX_ID_LED_0 || led_dat->id == PM8XXX_ID_LED_1)						// for 8960 platform !!!
		//if(led_dat->id == PM8XXX_ID_RGB_LED_RED || led_dat->id == PM8XXX_ID_RGB_LED_GREEN)		// for 8930 platform !!!
		{
			printk(LED_DRIVER_TAG "probe: device_create_file for %s led \n",led_dat->cdev.name);
		
			rc =   device_create_file(led_dat->cdev.dev, &dev_attr_delay_on);
			if (rc) {
				printk(LED_DRIVER_TAG "device_create_file on for %s led failed\n",led_dat->cdev.name);
				//goto err_out_attr_blink;
			}

			rc =   device_create_file(led_dat->cdev.dev, &dev_attr_delay_len);
			if (rc) {
				printk(LED_DRIVER_TAG "device_create_file off for %s led failed\n",led_dat->cdev.name);
				//goto err_out_attr_blink;
			}

			led_dat->cdev.blink_delay_on = 501;		//ms blink on time
			led_dat->cdev.blink_delay_off = 1001;		//blink period

			rc =   device_create_file(led_dat->cdev.dev, &dev_attr_blink);
			if (rc) {
				printk(LED_DRIVER_TAG "device_create_file blink for %s led failed\n",led_dat->cdev.name);
				//goto err_out_attr_blink;
			}
			led_dat->cdev.blink_brightness = 0;	//whether blink or not .  used as blink_flag !

		}

#else
		printk(LED_DRIVER_TAG "pm8xxx_led_probe() not use led_pwm mode\n");

#endif	/* CONFIG_LED_PWM_CTRL */
