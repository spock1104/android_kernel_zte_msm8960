...
#define ZTE_CONFIG_SCREEN_ON_WITHOUT_KEYOCDE   //ZTE
...
static int enable_to_shutdown=1;    //ZTE
...
static int pm_chg_ibatmax_set(struct pm8921_chg_chip *chip, int chg_current)
{
	...
	printk("ZTE pm8921 pm_chg_ibatmax_set=%d\n",chg_current);
...
#else   //ZTE
	if (minutes < PM8921_CHG_TCHG_MIN ) 
		{		
		pr_err("bad min minutes =%d asked to set\n", minutes);
		return -EINVAL;
		}
	else if(minutes > PM8921_CHG_TCHG_MAX)
		 {		
		 pr_err("bad max minutes =%d asked to set.setting 500\n", minutes);
		 minutes=PM8921_CHG_TCHG_MAX-12;
		 }	
#endif
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		/* USB charging */
		//if (usb_target_ma < USB_WALL_THRESHOLD_MA)
		if (usb_target_ma <= USB_WALL_THRESHOLD_MA)   //ZTE_ZHANGYU   ??
			val->intval = is_usb_chg_plugged_in(the_chip);
		else
		    return 0;
		break;
...
//printk("get_prop_batt_capacity run chip->battery_less_hardware=%d",chip->battery_less_hardware);//ZTE
	if (chip->battery_less_hardware)   //? ZTE
{
printk("get_prop_batt_capacity run chip->battery_less_hardware=%d",chip->battery_less_hardware);//ZTE
...
void pm8921_charger_vbus_draw(unsigned int mA)
{
	...
	else if (mA>500)
	{
	  ...
	  if(mA == 1500)//decrease Iusbin limit current
	  mA = 1500;   //JB large the charging current
	 }
#endif
...
{
        printk("pm8921 %s usb_target_ma:%d\n",__func__,usb_target_ma);
	if (usb_target_ma)
...
static irqreturn_t dcin_valid_irq_handler(int irq, void *data)
{
	struct pm8921_chg_chip *chip = data;
	int dc_present;
printk("dcin_valid_irq_handler"); //ZTE
	pm_chg_failed_clear(chip, 1);
			...
			int vbat_batt_terminal_uv, int ichg_meas_ma)
{
	...
	if (!is_ext_charging(chip)) {
		...
		printk("vddmax = %d vbat_batt_terminal_uv=%d\n",
	...
	printk("iterm_programmed = %d ichg_meas_ma=%d\n",
...
static void __devinit determine_initial_state(struct pm8921_chg_chip *chip)
{
	...
	printk("usb = %d, dc = %d  batt = %d state=%d\n",
			chip->usb_present,
			chip->dc_present,
			get_prop_batt_present(chip),
			fsm_state);
...
static int __devinit pm8921_chg_hw_init(struct pm8921_chg_chip *chip)
{
	...
	printk("ZTE chip->safe_current_ma=%d\n",chip->safe_current_ma);//ZTE_XJB_20130219
	...
	printk("pm8921_chg_hw_init fcc_uah=%d\n", fcc_uah);
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
	...
	chip->dc_psy.name = "ac",   //pm8921-dc -->ac  ZTE
